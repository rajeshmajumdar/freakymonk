...
def scan(url, redirect, useragent, postdata, proxy, dbfile):
    ...
    request.add_header('Origin', 'http://rajeshmajumdar.blogspot.com/')
    if postdata:
...
def check_url(url):
    ...
    if ((url_checked.scheme != 'http') & (url_checked.scheme != 'https')) | (url_checked.netloc == ''):
        raise argparse.ArgumentTypeError('Invalid %s URL (example: https://www.rajeshmajumdar.blogspot.com/path).' % url)
    return url_checked
...
def main():
    parser = argparse.ArgumentParser(description='Scanner for HTTP response headers.')
    ...
    parser.add_argument('-U', '--useragent', metavar='User-Agent', default='hsecscan', help='Set the User-Agent request header (default: rajeshmajumdar).')
    parser.add_argument('-D', '--dbfile', dest="dbfile", default='HTTPRes.db', type=lambda x: is_valid_file(parser, x), help='Set the database file (default: HTTPRes.db).')
    ...
    parser.add_argument('-x', '--proxy', help='Set the proxy server (example: 127.0.0.1:8080).')
